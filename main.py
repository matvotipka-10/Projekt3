...
def get_parties_results(url: str) -> dict[str, dict[str]]:
    """
    Returns a dictionary where the key is the municipality code
    and the value is a dictionary of party names and vote counts.
    """
    ...
    for div in find_div:
        ...
        for table in find_table:
            ...
            for tr in find_tr[2:]:
                ...
                name_count.update({find_td[1].text: clean(find_td[2].text)})
...
def get_municipality_codes_names(url: str) -> dict[str, dict[str]]:
    ...
    for table in find_table: 
        ...
        for tr in find_tr: 
            find_td = tr.find_all("td") 
            codes_names.update({find_td[0].text: 
                                {"code": find_td[0].text, 
                                "location": find_td[1].text}})
    return codes_names

def get_municipality_overview(url: str) -> dict[str, dict[str]]:
    """
    Returns a dictionary with numbers of registered voters, 
    envelopes issued and valid votes for one municipality.
    """
    response_get = requests.get(url)
    soup = BeautifulSoup(response_get.text, features="html.parser")
    find_table = soup.find("table", {"class": "table"})
    find_tr = find_table.find_all("tr")
    all_td = find_tr[2].find_all("td")
    get_key = parse.urlparse(url)
    key_list = get_key.query.split("&")
    key = key_list[2].split("=")
    overview = {key[1]: {"registered": clean(all_td[3].text),
                          "envelopes": clean(all_td[6].text), 
                          "valid": clean(all_td[7].text)}}
    return overview
...
def rows_for_csv(
    ...
    """
    Creates a dictionary of rows prepared for CSV output.
    """
    rows = {}
...
def save_rows_to_csv(rows: dict[str, dict], csv_name: str):
    """
    Saves prepared rows into csv file.
    """
...
def main():
    ...
    overviews = {}
    results = {}
    for one_url in new_url_addresses:
